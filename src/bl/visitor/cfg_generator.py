...
class CFGDigraph(Digraph):
    ...
    def add_node(self, name, _label, loop_node = None, _shape = None, _style = "filled"):
        if loop_node in ["L_C","DWL_C"]:
            _shape = 'diamond'
...
#             elif name == "Return":
#                 curr_node = self.__create_node(node_details, str(node_label), src_id, info)
#                 
#                 print("Return Node:",self.__curr_dgraph.get_return_node())
#                 self.__curr_dgraph.add_return_links(curr_node)
#                 
#                 self.__curr_dgraph.set_previous_nodes(None)
#                 node = None
#                 append_required = False
#             
#             elif node_label == "Break":
#                 print("Break Node:",self.__curr_dgraph.get_break_node())
# #                 self.__curr_dgraph.add_edge(self.__curr_dgraph.get_previous_nodes().get_name(), 
# #                                             self.__curr_dgraph.get_break_node().get_name(), _label = "break", _color = "red4",  _labelfontcolor = "red4   
#                 prev_node = self.__curr_dgraph.get_previous_nodes()
#                 print("Pre N", self.__curr_dgraph.get_info(), prev_node, self.__curr_dgraph.get_cbs(), self.__curr_dgraph.get_entry_node())
# #                 input("1")
#                 if not prev_node:
#                     if self.__curr_dgraph.get_cbs():
#                         self.__curr_dgraph.set_entry_node("break")
#                     #print("Akshay No operation")
#                 else:
#                     self.__curr_dgraph.add_break_link((prev_node,""))
#                     
#                 """
#                 break..... Link
#                 or entry link set just to indicate block is not empty... or contains only break or continue stmt
#                 """
# 
#                 self.__curr_dgraph.set_previous_nodes(None)
#                 node = None
#                 append_required = False
#             
#             elif node_label == "Continue":
#                 print("Continue in place::",self.__curr_dgraph.get_previous_nodes(), self.__curr_dgraph.get_continue_node())
#                 
#                 prev_node = self.__curr_dgraph.get_previous_nodes()
#                 print("Pre N", self.__curr_dgraph.get_info(), prev_node, self.__curr_dgraph.get_cbs(), self.__curr_dgraph.get_entry_node())
# #                 input()
#                 if not prev_node:
#                     if self.__curr_dgraph.get_cbs():
#                         self.__curr_dgraph.set_entry_node("continue")
#                     #print("Akshay No operation")
#                 else:
#                     self.__curr_dgraph.add_continue_link((prev_node,""))
#                 
#                 self.__curr_dgraph.set_previous_nodes(None)
#                 #self.__curr_dgraph.add_edge(prev, name, _label, _color)(self.__curr_dgraph.get_previous_nodes(), self.__curr_dgraph.get_join_node(), "continue", "green")
#                 
#                 node = None
#                 append_required = False   
            else:
        ...
        #node_details, str(cfg_id), src_id
    ...
    def draw_cfg(self, dgraph = None, src = True):
        file_name = 'cfg'
        if src:
            file_name += '_src'
        dgraph.render(file_name, "../../../lib/img/", False, False, 'png')
